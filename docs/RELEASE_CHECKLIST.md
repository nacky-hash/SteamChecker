...
- [x] スクリーンショットに実在のユーザー名・Steam ID・フルパスが写っていないこと
      （2026-09-12: `docs/images/scan-result.png` を README に掲載。
      一覧にはタイトル名・サイズ・見込み・根拠のみが出る列構成で、パスは表示されない。
      撮影は `PrintWindow` で対象ウィンドウのみを取得し、画像を目視で確認した。
      なお最初の撮影では `CopyFromScreen` を使ったため手前にあった別ウィンドウが写り、
      無関係なプロジェクト名が入った。**撮ったら必ず開いて中身を確認すること。**）

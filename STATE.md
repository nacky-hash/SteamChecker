...
2. **残っている未確認**（急がない）
   ...
   - **WPF 側の D-020 の実機確認**。ビルドは通っているが、中断通知の MessageBox と
     メモリ監視の発火を GUI で実際に見てはいない
   - **本物のメモリ枯渇での自主中断**。閾値 1 TB での人為的な発火は確認済みだが、
     実際にメモリが枯渇していく状況で間に合うかは未検証
   - **圧縮率推定の精度は再評価が必要**。2026-09-11 の測定は D-019 前のビルドで、
     見込み値が旧実装の二重計上値だった（`docs/STATUS.md` の訂正節）
3. **やらないと決めたこと**（蒸し返さない）
